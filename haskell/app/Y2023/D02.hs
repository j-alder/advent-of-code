...
-- [ (1, "3 blue, 4 red; 1 red") ]
-- [ (1, [ [(3, "blue"), (4, "red")], [(4,"red"), (1,"red"))] ] ) ]

type Results = [(String, Int)]
type Game = (Int, [Results]);
type Games = [Game];



fmtGames :: [String] -> Games
fmtGames input = do
  -- [ ["Game 1", "3 blue, 4 red; 1 red"] ]
  let input' = map (splitStr ':') input
  -- [ (1, ["3 blue, 4 red", "1 red"]) ]
  let input'' = map (\x -> (read (last (splitStr ' ' x)) :: Int, splitStr ';' (tail x))) input'
  -- [ (1, [ [(3, "blue"), (4, "red")], [(4,"red"), (1,"red"))] ] )
  let input''' = map (\(x, y) -> (x, map (\z -> (last z, read (head z) :: Int)) (splitStr ', ' y))) input''
  input'''
...
soln input = do
  let input' = splitStr '\n' input
  let games = fmtGames input'
  (show (part1 games), show (part2 games))
