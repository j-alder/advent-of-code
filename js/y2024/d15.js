...
inspection, they seem quite anxious, so you drive your mini submarine over to see 
if you can help.
...
robot will attempt to make (your puzzle input). The problem is that the movements 
will sometimes fail as boxes are shifted around, making the actual movements of 
the robot difficult to predict.
...
The larger example has many more moves; after the robot has finished those moves, 
...
This warehouse's layout is surprisingly similar to the one you just helped. There 
is one key difference: everything except the robot is twice as wide! The robot's 
list of movements doesn't change.
...
speed, boxes can be aligned such that they directly push two other boxes at once. 
...
After appropriately resizing this map, the robot would push around these boxes as 
follows:
...
distances are measured from the edge of the map to the closest edge of the box in 
question. So, the box shown below has a distance of 1 from the top edge of the 
map and 5 from the left edge of the map, resulting in a GPS coordinate of 100 * 
1 + 5 = 105.
